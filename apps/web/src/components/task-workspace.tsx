...
export function TaskWorkspace({ locale }: { locale: Locale }) {
  ...
  const [minutes, setMinutes] = useState(demoTask.minutes);
  ...
  async function lowerDifficulty() {
    if (easier) return;
    if (!navigator.onLine) { setError(zh ? "离线时不能更改服务器任务难度。" : "Task difficulty cannot be changed on the server while offline."); setPhase("error"); return; }
    if (!demo) { setError(zh ? "难度调整尚未被服务器确认。" : "The difficulty change has not been confirmed by the server."); setPhase("error"); return; }
    await new Promise((resolve) => window.setTimeout(resolve, 300));
    setMinutes(15); setEasier(true);
  }
  ...
  return <div className="task-page"><aside className="task-brief"><a href={`/${locale}/today`} className="inline-link"><ArrowLeft />{zh ? "今日一步" : "Today"}</a><p className="eyebrow">{zh ? "任务简报" : "Task brief"}</p><h1>{demoTask.title}</h1><p>{demoTask.why}</p><div className="brief-block"><strong>{zh ? "完成标准" : "Definition of done"}</strong><p>{demoTask.judgment}</p></div><div className="task-facts"><span><Clock3 /> {minutes} {zh ? "分钟" : "min"}</span><span><FileCode2 /> {zh ? "代码或文字" : "Code or writing"}</span></div><button className="button ghost difficulty-button" type="button" onClick={lowerDifficulty} disabled={easier}>{easier ? (zh ? "已调整为更容易" : "Adjusted to easier") : (zh ? "降低难度" : "Make this easier")}</button><div className="coach-nudge"><Sparkles /><p><strong>Coach</strong><br />{zh ? "先写出允许的转换，再问谁拥有改变状态的权限。" : "Write allowed transitions first, then ask who owns the authority to change state."}</p></div></aside>
