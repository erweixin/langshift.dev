...
export function RouteReview({ locale, from, to }: { locale: Locale; from: string; to: string }) {
  ...
  const [bridge, setBridge] = useState(zh ? "从界面状态与异步流，迁移到可恢复的 Agent 运行生命周期" : "From UI state and async flows to recoverable agent run lifecycles");
  ...
  return <main className="route-shell" id="main-content">
    ...
    <section className="bridge-card card"><div><p className="section-label">{zh ? "迁移桥" : "Capability bridge"}{corrected ? ` · ${zh ? "已由你修正" : "Corrected by you"}` : ""}</p>{editingBridge ? <div className="bridge-editor"><label className="field"><span>{zh ? "修正迁移关系" : "Correct the transfer relationship"}</span><textarea name="bridge-correction" autoComplete="off" value={bridgeDraft} onChange={(event) => setBridgeDraft(event.target.value)} /></label><div><button className="button ghost" type="button" onClick={() => { setBridgeDraft(bridge); setEditingBridge(false); }}>{zh ? "取消" : "Cancel"}</button><button className="button primary" type="button" disabled={!bridgeDraft.trim()} onClick={() => { setBridge(bridgeDraft.trim()); setCorrected(true); setEditingBridge(false); }}>{zh ? "保存修正" : "Save correction"}</button></div></div> : <><h2>{bridge}</h2><p>{zh ? "共同点是状态转换；关键差异是进程崩溃、重试和不可逆副作用。" : "The shared primitive is state transition. The hard difference is surviving crashes, retries, and irreversible effects."}</p></>}</div><button type="button" className="icon-button" aria-label={zh ? "编辑迁移桥" : "Edit bridge"} aria-expanded={editingBridge} onClick={() => setEditingBridge(true)}><Pencil /></button></section>
