...
export function SettingsPanel({ locale }: { locale: Locale }) {
  ...
  return <div className="page-wrap"><header className="page-head"><div><p className="eyebrow">{zh ? "设置" : "Settings"}</p><h1>{zh ? "你的工作区，由你掌控。" : "Your workspace, under your control."}</h1></div></header><div className="settings-layout"><nav className="settings-nav" aria-label={zh ? "设置分区" : "Settings sections"}>{sections.map(({ id, icon: Icon, label }) => <a href={`#${id}`} key={id}><Icon />{label}</a>)}</nav><div className="settings-content">
    ...
    <section className="card" id="privacy"><p className="section-label">{zh ? "隐私与数据" : "Privacy & data"}</p><div className="setting-row"><MessageSquareText /><div><strong>{zh ? "Support Center" : "Support Center"}</strong><p>{zh ? "创建加密工单，查看支持等级、SLA 时钟与完整回复记录。" : "Create encrypted cases and review support tier, SLA clocks, and the full reply history."}</p></div><a className="button" href={`/${locale}/support`}>{zh ? "打开" : "Open"}</a></div><div className="setting-row"><FileArchive /><div><strong>{zh ? "导出全部数据" : "Export all data"}</strong><p>{exportState === "done" ? (zh ? "导出请求已确认；准备好后会提供限时下载。" : "Export confirmed; an expiring download appears when ready.") : (zh ? "获取 Mission、任务、证据、项目、Memory 与审计记录的可移植副本。" : "Get a portable copy of Missions, tasks, evidence, projects, Memory, and audit records.")}</p></div><button className="button" type="button" onClick={requestExport} disabled={exportState === "working"}>{exportState === "done" ? (zh ? "已请求" : "Requested") : (zh ? "请求导出" : "Request export")}</button></div><div className="setting-row" id="legal"><ShieldCheck /><div><strong>{zh ? "隐私与法律说明" : "Privacy and legal notices"}</strong><p>AGPL-3.0-only · Source · No warranty · Trademarks · Security</p></div><a className="button" href={`/${locale}/legal`}>{zh ? "查看" : "Review"}</a></div><form className="deletion-form danger-zone" onSubmit={requestDeletion}><Trash2 /><div><strong>{zh ? "删除账户与托管数据" : "Delete account and hosted data"}</strong><p>{deleteState === "done" ? (zh ? "删除请求已创建，宽限期内仍可撤销。" : "Erasure request created; it remains cancelable during the grace period.") : (zh ? "需要最近重新认证。输入 DELETE MY ACCOUNT 创建可撤销的删除请求。" : "Recent reauthentication is required. Enter DELETE MY ACCOUNT to create a cancelable erasure request.")}</p><label className="field"><span>{zh ? "确认短语" : "Confirmation phrase"}</span><input name="delete-confirmation" autoComplete="off" value={deletePhrase} onChange={(event) => setDeletePhrase(event.target.value)} /></label></div><button className="button danger" type="submit" disabled={deletePhrase !== "DELETE MY ACCOUNT" || deleteState === "working"}>{deleteState === "done" ? (zh ? "已创建请求" : "Request created") : (zh ? "请求删除" : "Request erasure")}</button></form><div className="setting-row danger-zone"><LogOut /><div><strong>{zh ? "退出登录" : "Sign out"}</strong><p>{zh ? "本机未提交草稿不会同步到其他设备。" : "Unsubmitted device drafts do not move to another device."}</p></div><button className="button danger" type="button">{zh ? "退出" : "Sign out"}</button></div></section>
  </div></div></div>;
}
