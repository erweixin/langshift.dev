...
func TestEmailChangeIsVersionedConfirmedRotatingAndSecretSafe(t *testing.T) {
	...
	service := AuthService{
		...
		PublicTenantID: publicTenantID, StoreEpoch: storeEpoch, Region: "US", VerificationTTL: 24 * time.Hour, PasswordResetTTL: 30 * time.Minute, EmailChangeTTL: 24 * time.Hour, SessionTTL: 30 * 24 * time.Hour, IdempotencyTTL: 24 * time.Hour,
