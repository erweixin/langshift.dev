...
func TestPasswordRecoveryAndChangeAreUniformSingleUseRotatingAndSecretSafe(t *testing.T) {
	...
	service := AuthService{
		...
		InvitationTokens:  opaque.Manager{Purpose: "invitation", Pepper: bytes.Repeat([]byte{0x8c}, 32)},
		...
		PublicTenantID: publicTenantID, StoreEpoch: "40000000-0000-0000-0000-000000000801", Region: "US", VerificationTTL: 24 * time.Hour, PasswordResetTTL: 30 * time.Minute, EmailChangeTTL: 24 * time.Hour, ReauthenticationTTL: 15 * time.Minute, ErasureGracePeriod: 7 * 24 * time.Hour, SessionTTL: 30 * 24 * time.Hour, IdempotencyTTL: 24 * time.Hour,
