...
func TestEffectOutcomeUnknownCompletesAttemptWithoutJoiningGroup(t *testing.T) {
	...
	var runStatus, toolStatus, inboxStatus, attemptStatus, jobStatus, effectStatus, toolResultEvent, effectResultEvent string
	var runVersion, toolVersion, effectVersion, continuations, outbox int
	var groupJoined bool
	var actualReconcileAt time.Time
	err = admin.QueryRow(ctx, `SELECT r.status,r.run_version,t.status,t.tool_call_version,t.result_event_id::text,i.status,a.status,j.status,e.status,e.version,e.result_event_id::text,e.reconciliation_due_at,g.joined,(SELECT count(*) FROM agent.continuations WHERE tenant_id=$1 AND group_id=$3),(SELECT count(*) FROM agent.outbox WHERE tenant_id=$1) FROM agent.runs r JOIN agent.tool_calls t ON t.run_id=r.id JOIN agent.inbox i ON i.id=$4 JOIN agent.job_attempts a ON a.id=$5 JOIN agent.jobs j ON j.id=$6 JOIN agent.tool_effects e ON e.tool_call_id=t.id JOIN agent.parallel_groups g ON g.id=$3 WHERE r.tenant_id=$1 AND r.id=$2`, tenantID, runID, requested.GroupID, claim.InboxID, claim.AttemptID, claim.JobID).Scan(&runStatus, &runVersion, &toolStatus, &toolVersion, &toolResultEvent, &inboxStatus, &attemptStatus, &jobStatus, &effectStatus, &effectVersion, &effectResultEvent, &actualReconcileAt, &groupJoined, &continuations, &outbox)
	if err != nil {
		t.Fatal(err)
	}
	if runStatus != "waiting_tool" || runVersion != 4 || toolStatus != "outcome_unknown" || toolVersion != 3 || toolResultEvent == "" || toolResultEvent != effectResultEvent || inboxStatus != "completed" || attemptStatus != "succeeded" || jobStatus != "succeeded" || effectStatus != "outcome_unknown" || effectVersion != 3 || !actualReconcileAt.Equal(reconcileAt) || groupJoined || continuations != 0 || outbox != 12 {
		t.Fatalf("run=%s/v%d tool=%s/v%d events=%s/%s inbox=%s attempt=%s job=%s effect=%s/v%d reconcile=%s joined=%v continuations=%d outbox=%d", runStatus, runVersion, toolStatus, toolVersion, toolResultEvent, effectResultEvent, inboxStatus, attemptStatus, jobStatus, effectStatus, effectVersion, actualReconcileAt, groupJoined, continuations, outbox)
	}
}
