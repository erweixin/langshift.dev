...
func (store SchedulerStore) finishDispatch(ctx context.Context, claim DispatchClaim, errorCode string, published bool) error {
	...
	if published {
		tag, err = tx.Exec(ctx, `UPDATE agent.jobs SET dispatch_lease_hash=NULL,dispatch_lease_expires_at=NULL,last_dispatched_at=$1,available_at=CASE WHEN status='pending' THEN $2 ELSE available_at END,delivery_due_at=CASE WHEN status IN ('pending','running') THEN $2 ELSE NULL END,redelivery_requested_at=NULL,redelivery_reason=NULL,last_error_code=NULL,updated_at=$1 WHERE id=$3 AND tenant_id=$4 AND command_id=$5 AND dispatch_version=$6 AND status IN ('pending','running','succeeded','failed','cancelled','expired','dead_letter') AND ((dispatch_lease_hash=$7 AND dispatch_lease_expires_at>$1) OR (dispatch_lease_hash IS NULL AND dispatch_lease_expires_at IS NULL))`, now, now.Add(store.RedeliveryDelay), claim.Candidate.ID, claim.Candidate.TenantID, claim.Candidate.Command.CommandID, claim.Candidate.DispatchVersion+1, digest[:])
	} else {
		tag, err = tx.Exec(ctx, `UPDATE agent.jobs SET dispatch_lease_hash=NULL,dispatch_lease_expires_at=NULL,available_at=CASE WHEN redelivery_requested_at IS NULL THEN $1 ELSE available_at END,redelivery_requested_at=CASE WHEN redelivery_requested_at IS NULL THEN NULL ELSE $1 END,last_error_code=$2,updated_at=$3 WHERE id=$4 AND tenant_id=$5 AND command_id=$6 AND dispatch_version=$7 AND status IN ('pending','running') AND dispatch_lease_hash=$8 AND dispatch_lease_expires_at>$3`, now.Add(store.RetryDelay), errorCode, now, claim.Candidate.ID, claim.Candidate.TenantID, claim.Candidate.Command.CommandID, claim.Candidate.DispatchVersion+1, digest[:])
	}
