...
func TestProductionManifestIsSequentialAndContentAddressed(t *testing.T) {
	...
	if len(manifest.Migrations) != 74 {
		t.Fatalf("production migration count=%d, want 74", len(manifest.Migrations))
	}
	latest := manifest.Migrations[len(manifest.Migrations)-1]
	if latest.Version != 74 || latest.Name != "portfolio_export_finalization" || !latest.Reversible {
