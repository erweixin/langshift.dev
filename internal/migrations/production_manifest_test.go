...
func TestProductionManifestIsSequentialAndContentAddressed(t *testing.T) {
	...
	if len(manifest.Migrations) != 24 {
		t.Fatalf("production migration count=%d, want 24", len(manifest.Migrations))
	}
	latest := manifest.Migrations[len(manifest.Migrations)-1]
	if latest.Version != 24 || latest.Name != "artifact_revision_protocol" || !latest.Reversible {
