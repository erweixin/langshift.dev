...
func TestProductionManifestIsSequentialAndContentAddressed(t *testing.T) {
	...
	if len(manifest.Migrations) != 49 {
		t.Fatalf("production migration count=%d, want 49", len(manifest.Migrations))
	}
	latest := manifest.Migrations[len(manifest.Migrations)-1]
	if latest.Version != 49 || latest.Name != "direct_tool_approval_protocol" || !latest.Reversible {
