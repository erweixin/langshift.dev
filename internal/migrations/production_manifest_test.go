...
func TestProductionManifestIsSequentialAndContentAddressed(t *testing.T) {
	...
	if len(manifest.Migrations) != 41 {
		t.Fatalf("production migration count=%d, want 41", len(manifest.Migrations))
	}
	latest := manifest.Migrations[len(manifest.Migrations)-1]
	if latest.Version != 41 || latest.Name != "run_cancellation_recovery_discovery" || !latest.Reversible {
