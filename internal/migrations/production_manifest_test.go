...
func TestProductionManifestIsSequentialAndContentAddressed(t *testing.T) {
	...
	if len(manifest.Migrations) != 23 {
		t.Fatalf("production migration count=%d, want 23", len(manifest.Migrations))
	}
	latest := manifest.Migrations[len(manifest.Migrations)-1]
	if latest.Version != 23 || latest.Name != "workspace_reconciliation_lifecycle" || !latest.Reversible {
