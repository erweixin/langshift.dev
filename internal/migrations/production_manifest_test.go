...
func TestProductionManifestIsSequentialAndContentAddressed(t *testing.T) {
	...
	if len(manifest.Migrations) != 19 {
		t.Fatalf("production migration count=%d, want 19", len(manifest.Migrations))
	}
	latest := manifest.Migrations[len(manifest.Migrations)-1]
	if latest.Version != 19 || latest.Name != "approval_scope_drift" || !latest.Reversible {
