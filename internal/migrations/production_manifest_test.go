...
func TestProductionManifestIsSequentialAndContentAddressed(t *testing.T) {
	...
	if len(manifest.Migrations) != 42 {
		t.Fatalf("production migration count=%d, want 42", len(manifest.Migrations))
	}
	latest := manifest.Migrations[len(manifest.Migrations)-1]
	if latest.Version != 42 || latest.Name != "runtime_run_cancellation_authority" || !latest.Reversible {
