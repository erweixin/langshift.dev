...
```rust !! rs
// Mutable borrow (similar to JavaScript's writable reference)
...
fn main() {
    ...
    // Restrictions of mutable borrow
    let mut s = String::from("hello");
    
    let r1 = &mut s; // First mutable borrow
    // let r2 = &mut s; // Error: cannot have multiple mutable borrows at the same time
    // let r3 = &s;     // Error: cannot have mutable and immutable borrows at the same time
    
    println!("{}", r1); // Use r1
    // r1 leaves scope here
    
    let r2 = &mut s; // Now a second mutable borrow can be created
    // let r3 = &s;   // Error: cannot have mutable and immutable borrows at the same time, need to wait until r2 ends
    println!("{}", r2);
    let r3 = &s; // Correct: now an immutable borrow can be created
    println!("{}", r3);
}
```
</UniversalEditor>
