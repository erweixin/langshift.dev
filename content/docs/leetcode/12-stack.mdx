...
```rust
struct MinStack { st: Vec<i32>, min_st: Vec<i32> }

impl MinStack {
    fn new() -> Self { MinStack { st: vec![], min_st: vec![] } }
    fn push(&mut self, val: i32) {
        self.st.push(val);
        let cur_min = self.min_st.last().map_or(val, |&m| m.min(val));
        self.min_st.push(cur_min);
    }
    fn pop(&mut self) { self.st.pop(); self.min_st.pop(); }
    fn top(&self) -> i32 { *self.st.last().unwrap() }
    fn get_min(&self) -> i32 { *self.min_st.last().unwrap() }
}

fn main() {
    let mut ms = MinStack::new();
    ms.push(-2); ms.push(0); ms.push(-3);
    println!("{}", ms.get_min()); // -3
    ms.pop();
    println!("{}", ms.top());     // 0
    println!("{}", ms.get_min()); // -2
}
```
...
fn decode_string(s: &str) -> String {
    ...
    for ch in s.chars() {
        if ch.is_ascii_digit() {
            cur_num = cur_num * 10 + (ch as usize - '0' as usize);
        } else if ch == '[' {
            num_stack.push(cur_num);
            str_stack.push(cur_str.clone());
            cur_num = 0; cur_str = String::new();
        } else if ch == ']' {
            let k = num_stack.pop().unwrap();
            let prev = str_stack.pop().unwrap();
            cur_str = prev + &cur_str.repeat(k);
        } else {
            cur_str.push(ch);
        }
    }
...
fn main() {
    ...
    println!("{}", decode_string("10[a]"));         // "aaaaaaaaaa"
    println!("{}", decode_string("abc3[cd]xyz"));   // "abccdcdcdxyz"
}
```
...
fn daily_temperatures(temperatures: Vec<i32>) -> Vec<i32> {
    ...
    for i in 0..n {
        while st.last().map_or(false, |&j| temperatures[i] > temperatures[j]) {
            let j = st.pop().unwrap();
            ans[j] = (i - j) as i32;
        }
        st.push(i);
...
fn main() {
    println!("{:?}", daily_temperatures(vec![73,74,75,71,69,72,76,73])); // [1,1,4,2,1,1,0,0]
    println!("{:?}", daily_temperatures(vec![30,40,50,60]));              // [1,1,1,0]
    println!("{:?}", daily_temperatures(vec![30,60,90]));                 // [1,1,0]
    println!("{:?}", daily_temperatures(vec![90,80,70]));                 // [0,0,0]
    println!("{:?}", daily_temperatures(vec![70,70,70,71]));              // [3,2,1,0]
}
```
...
fn largest_rectangle_area(heights: Vec<i32>) -> i32 {
    let mut h = vec![0i32];
    h.extend_from_slice(&heights);
    h.push(0);
    let mut st: Vec<usize> = vec![];
    let mut best = 0;
    for i in 0..h.len() {
        while st.last().map_or(false, |&j| h[i] < h[j]) {
            let mid = st.pop().unwrap();
            let left = *st.last().unwrap_or(&0);
            let width = (i - left - 1) as i32;
            best = best.max(h[mid] * width);
        }
        st.push(i);
    }
    best
...
fn main() {
    ...
    println!("{}", largest_rectangle_area(vec![1,1,1,1]));          // 4
    println!("{}", largest_rectangle_area(vec![6,2,5,4,5,1,6]));   // 12
