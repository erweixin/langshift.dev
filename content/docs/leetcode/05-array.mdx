...
fn main() {
    ...
    println!("{}", max_sub_array(vec![-1, -2, -3]));                     // -1
    println!("{}", max_sub_array(vec![0, 0, 0]));                        // 0
}
```
...
fn main() {
    println!("{:?}", merge(vec![vec![1,3],vec![2,6],vec![8,10],vec![15,18]])); // [[1,6],[8,10],[15,18]]
    println!("{:?}", merge(vec![vec![1,4],vec![4,5]]));                         // [[1,5]]
    println!("{:?}", merge(vec![]));                                             // []
    println!("{:?}", merge(vec![vec![1,4]]));                                   // [[1,4]]
    println!("{:?}", merge(vec![vec![1,4],vec![0,2],vec![3,5]]));               // [[0,5]]
}
```
...
fn main() {
    let cases = vec![(vec![1,2,3,4,5,6,7], 3), (vec![-1,-100,3,99], 2), (vec![1,2], 0), (vec![1,2], 3), (vec![1], 100)];
    for (mut nums, k) in cases {
        rotate(&mut nums, k);
        println!("{:?}", nums);
    }
}
...
fn product_except_self(nums: Vec<i32>) -> Vec<i32> {
    ...
    let mut left = 1;
    for i in 0..n { res[i] = left; left *= nums[i]; }
    let mut right = 1;
    for i in (0..n).rev() { res[i] *= right; right *= nums[i]; }
    res
}
...
fn main() {
    ...
    println!("{:?}", product_except_self(vec![2, 3]));             // [3,2]
    println!("{:?}", product_except_self(vec![5]));                // [1]
}
```
...
fn first_missing_positive(mut nums: Vec<i32>) -> i32 {
    ...
    while i < n {
        let x = nums[i];
        let j = (x - 1) as usize;
        if x < 1 || x > n as i32 || nums[j] == x { i += 1; continue; }
        nums.swap(i, j);
    }
    for i in 0..n {
...
fn main() {
    println!("{}", first_missing_positive(vec![1, 2, 0]));         // 3
    println!("{}", first_missing_positive(vec![3, 4, -1, 1]));     // 2
    println!("{}", first_missing_positive(vec![7, 8, 9, 11, 12])); // 1
    println!("{}", first_missing_positive(vec![1, 1]));            // 2
    println!("{}", first_missing_positive(vec![2]));               // 1
}
```
