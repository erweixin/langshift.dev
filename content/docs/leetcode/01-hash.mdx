...
fn two_sum(nums: Vec<i32>, target: i32) -> Vec<i32> {
    ...
    for (i, &x) in nums.iter().enumerate() {
        let need = target - x;
        if let Some(&j) = index_by_value.get(&need) {
...
fn main() {
    ...
    println!("{:?}", two_sum(vec![-1, -2, -3, -4, -5], -8));       // [2, 4]
    println!("{:?}", two_sum(vec![0, 4, 3, 0], 0));                // [0, 3]
}
```
...
fn group_anagrams(strs: Vec<String>) -> Vec<Vec<String>> {
    ...
    for s in strs {
        ...
        groups.entry(cnt).or_default().push(s);
    }
    groups.into_values().collect()
}

fn main() {
    let s1: Vec<String> = vec!["eat","tea","tan","ate","nat","bat"].into_iter().map(String::from).collect();
    println!("{:?}", group_anagrams(s1));
    println!("{:?}", group_anagrams(vec!["".to_string()]));
    println!("{:?}", group_anagrams(vec!["a".to_string()]));
    let s4: Vec<String> = vec!["ab","ba","abc","cab","bca","bbb"].into_iter().map(String::from).collect();
    println!("{:?}", group_anagrams(s4));
    let s5: Vec<String> = vec!["listen","silent","enlist","google","gogole"].into_iter().map(String::from).collect();
    println!("{:?}", group_anagrams(s5));
}
```
...
fn main() {
    ...
    println!("{}", longest_consecutive(vec![0, 3, 7, 2, 5, 8, 4, 6, 0, 1]));      // 9
    println!("{}", longest_consecutive(vec![]));                                    // 0
    println!("{}", longest_consecutive(vec![1, 2, 0, 1]));                         // 3
    println!("{}", longest_consecutive(vec![9, 1, 4, 7, 3, -1, 0, 5, 8, -1, 6])); // 7
}
```
