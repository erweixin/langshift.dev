...
fn main() {
    ...
    println!("{}", length_of_longest_substring(""));          // 0
    println!("{}", length_of_longest_substring("abba"));     // 2
}
```
...
func main() {
	...
	fmt.Println(findAnagrams("aaaaa", "aa"))      // [0 1 2 3]
	fmt.Println(findAnagrams("a", "ab"))          // []
...
fn find_anagrams(s: &str, p: &str) -> Vec<i32> {
    ...
    for i in 0..m {
        ...
        diff[(p[i] - b'a') as usize] -= 1;
    }
    let mut non_zero = diff.iter().filter(|&&x| x != 0).count() as i32;
    let mut ans = vec![];
    if non_zero == 0 { ans.push(0); }
    for i in m..n {
        let in_idx = (s[i] - b'a') as usize;
        let out_idx = (s[i - m] - b'a') as usize;
        if diff[in_idx] == 0 { non_zero += 1; }
        diff[in_idx] += 1;
        if diff[in_idx] == 0 { non_zero -= 1; }
        if diff[out_idx] == 0 { non_zero += 1; }
        diff[out_idx] -= 1;
        if diff[out_idx] == 0 { non_zero -= 1; }
        if non_zero == 0 { ans.push((i - m + 1) as i32); }
    }
    ans
}
...
fn main() {
    ...
    println!("{:?}", find_anagrams("aaaaa", "aa"));       // [0, 1, 2, 3]
    println!("{:?}", find_anagrams("a", "ab"));           // []
    println!("{:?}", find_anagrams("baa", "aa"));         // [1]
}
```
