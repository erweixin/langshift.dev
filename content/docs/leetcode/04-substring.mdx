...
fn subarray_sum(nums: Vec<i32>, k: i32) -> i32 {
    let mut count_by_prefix: HashMap<i32, i32> = HashMap::new();
    count_by_prefix.insert(0, 1);
    let (mut prefix, mut ans) = (0, 0);
    for x in nums {
        prefix += x;
        ans += count_by_prefix.get(&(prefix - k)).copied().unwrap_or(0);
...
fn main() {
    ...
    println!("{}", subarray_sum(vec![0, 0, 0], 0));    // 6
...
fn max_sliding_window(nums: Vec<i32>, k: usize) -> Vec<i32> {
    if nums.is_empty() || k == 0 { return vec![]; }
    let n = nums.len();
    let mut dq: VecDeque<usize> = VecDeque::new();
    let mut ans = vec![];
    for i in 0..n {
        while dq.back().map_or(false, |&j| nums[j] <= nums[i]) { dq.pop_back(); }
        dq.push_back(i);
        if *dq.front().unwrap() + k <= i { dq.pop_front(); }
        if i >= k - 1 { ans.push(nums[*dq.front().unwrap()]); }
    }
    ans
}

fn main() {
    println!("{:?}", max_sliding_window(vec![1, 3, -1, -3, 5, 3, 6, 7], 3)); // [3,3,5,5,6,7]
    println!("{:?}", max_sliding_window(vec![1], 1));                          // [1]
    println!("{:?}", max_sliding_window(vec![9, 8, 7, 6], 2));                // [9,8,7]
    println!("{:?}", max_sliding_window(vec![4, 4, 4, 4], 2));                // [4,4,4]
    println!("{:?}", max_sliding_window(vec![1, -1], 1));                      // [1,-1]
}
```
...
fn min_window(s: &str, t: &str) -> String {
    ...
    for ch in t.chars() { *need.entry(ch).or_insert(0) += 1; }
    let mut missing = t.len() as i32;
    let (mut left, mut best_l, mut best_len) = (0, 0, usize::MAX);
    for (right, &ch) in s.iter().enumerate() {
        ...
        while missing == 0 {
            ...
            if len < best_len { best_len = len; best_l = left; }
            let out = s[left];
            if let Some(cnt) = need.get_mut(&out) {
                *cnt += 1;
                if *cnt > 0 { missing += 1; }
            }
            left += 1;
    ...
    if best_len == usize::MAX { String::new() } else { s[best_l..best_l + best_len].iter().collect() }
}
...
fn main() {
    ...
    println!("{}", min_window("aa", "aa"));              // "aa"
    println!("{}", min_window("bba", "ab"));             // "ba"
}
```
