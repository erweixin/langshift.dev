...
fn main() {
    ...
    println!("{}", generate(5).first().map(|r| format!("{:?}", r)).unwrap_or_default().starts_with('[')); // true
    println!("{}", generate(0).len()); // 0
...
fn num_squares(n: i32) -> i32 {
    ...
    dp[0] = 0;
    let mut i = 1i32;
    while (i as usize) * (i as usize) <= n {
        let sq = (i * i) as usize;
        for x in sq..=n {
            if dp[x - sq] != i32::MAX {
                dp[x] = dp[x].min(dp[x - sq] + 1);
            }
        }
        i += 1;
    }
...
fn main() {
    ...
    println!("{}", num_squares(1));   // 1
    println!("{}", num_squares(43));  // 3
    println!("{}", num_squares(100)); // 1
}
```
...
fn main() {
    ...
    println!("{}", coin_change(vec![1, 3, 4], 6));                     // 2
    println!("{}", coin_change(vec![186, 419, 83, 408], 6249));        // 20
}
```
...
fn word_break(s: String, word_dict: Vec<String>) -> bool {
    ...
    for i in 1..=n {
        for j in 0..i {
            ...
            if dict.contains(std::str::from_utf8(&bytes[j..i]).unwrap()) {
                dp[i] = true;
                break;
            }
        }
...
fn main() {
    ...
    println!("{}", word_break("applepenapple".into(), vec!["apple".into(), "pen".into()]));
    println!("{}", word_break("catsandog".into(), vec!["cats".into(), "dog".into(), "sand".into(), "and".into(), "cat".into()]));
    println!("{}", word_break("".into(), vec!["a".into()]));
    println!("{}", word_break("aaaaaaa".into(), vec!["aaaa".into(), "aaa".into()]));
}
```
...
```rust
fn lower_bound(tails: &[i32], x: i32) -> usize {
    let mut l = 0usize;
    let mut r = tails.len();
    while l < r {
        let mid = (l + r) / 2;
...
fn main() {
    ...
    println!("{}", length_of_lis(vec![1, 2, 3, 4, 5]));                // 5
    println!("{}", length_of_lis(vec![5, 4, 3, 2, 1]));                // 1
}
```
...
fn max_product(nums: Vec<i32>) -> i32 {
    ...
    let mut max_here = 1i32;
    let mut min_here = 1i32;
    for x in nums {
        if x < 0 {
            std::mem::swap(&mut max_here, &mut min_here);
        }
...
fn main() {
    ...
    println!("{}", max_product(vec![-1, -2, -9, -6]));   // 108
}
```
...
fn main() {
    ...
    println!("{}", can_partition(vec![2, 2, 3, 5]));    // false
    println!("{}", can_partition(vec![3, 3, 3, 4, 5])); // true
    println!("{}", can_partition(vec![1]));            // false
}
```
...
fn longest_valid_parentheses(s: String) -> i32 {
    ...
    for i in 1..n {
        if s[i] == b')' {
            let j = i as i32 - 1 - dp[i - 1];
            if j >= 0 && s[j as usize] == b'(' {
                dp[i] = dp[i - 1] + 2;
                if j > 0 {
                    dp[i] += dp[(j - 1) as usize];
                }
            }
        }
        best = best.max(dp[i]);
...
fn main() {
    println!("{}", longest_valid_parentheses("(()".into()));       // 2
    println!("{}", longest_valid_parentheses(")()())".into()));     // 4
    println!("{}", longest_valid_parentheses("".into()));           // 0
    println!("{}", longest_valid_parentheses("()(()".into()));    // 2
    println!("{}", longest_valid_parentheses("()(())".into()));   // 6
}
```
