...
// C++ 類型主要在編譯時檢查，但也支援運行時類型資訊 (RTTI)
#include <typeinfo> // 需要包含此標頭檔以使用 typeid
// std::cout << typeid(age).name() << std::endl; // 列印類型名稱
...
C++ 中的函式通常需要明確的返回類型和參數類型，但也可以透過 `auto` 關鍵字（C++11）進行類型推斷，或使用模板實現泛型函式。
...
// 使用 auto 進行返回類型推斷 (C++14)
auto multiply(int a, int b) {
  return a * b; // 返回類型推斷為 int
}
...
C++ 使用 `#include` 預處理器指令來包含標頭檔，類似於 JavaScript 中的 `import` 語句。C++ 的命名空間用於組織程式碼並防止命名衝突。

<UniversalEditor title="模組匯入對比" compare={true}>
```javascript !! js
// JavaScript: 模組匯入
// 檔案: math.js
