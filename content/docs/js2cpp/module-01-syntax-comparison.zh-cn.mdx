...
// C++ 类型主要在编译时检查，但也支持运行时类型信息 (RTTI)
#include <typeinfo> // 需要包含此头文件以使用 typeid
// std::cout << typeid(age).name() << std::endl; // 打印类型名称
...
C++ 中的函数通常需要明确的返回类型和参数类型，但也可以通过 `auto` 关键字（C++11）进行类型推断，或使用模板实现泛型函数。
...
// 使用 auto 进行返回类型推断 (C++14)
auto multiply(int a, int b) {
  return a * b; // 返回类型推断为 int
}
...
C++ 使用 `#include` 预处理器指令来包含头文件，类似于 JavaScript 中的 `import` 语句。C++ 的命名空间用于组织代码并防止命名冲突。

<UniversalEditor title="模块导入对比" compare={true}>
```javascript !! js
// JavaScript: 模块导入
// 文件: math.js
