...
// C++ types are primarily checked at compile time, but also supports Runtime Type Information (RTTI)
#include <typeinfo> // Required header for using typeid
// std::cout << typeid(age).name() << std::endl; // Print type name
...
// Using auto for return type deduction (C++14)
auto multiply(int a, int b) {
  return a * b; // Return type deduced as int
}

// Using templates for generic functions
template <typename T>
...
// std::cout << addGeneric(5, 3) << std::endl; // int
// std::cout << addGeneric(5.5, 3.2) << std::endl; // double
```
</UniversalEditor>
...
<UniversalEditor title="Module Import Comparison" compare={true}>
```javascript !! js
...
```cpp !! cpp
// C++: Header File Inclusion and Namespaces
// file: math.h (or math.hpp)
#ifndef MATH_H
#define MATH_H
...
// file: main.cpp
#include <iostream>
#include "math.h" // Include custom header file
