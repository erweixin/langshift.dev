...
### 2.5 数据类 (Data Classes)

在 JavaScript 中，我们经常使用对象字面量或接口（TypeScript）来存储数据。在 Python 3.7+ 中，`dataclasses` 提供了一种类似且类型安全的方式来定义数据持有类，而无需编写繁琐的 `__init__` 方法。

<PythonEditor title="Data Classes 对比" compare={true}>
```javascript !! js
...
# 比较
user2 = User(1, "张三", "zhangsan@example.com")
print(user == user2)  # True (自动实现值比较)
```
</PythonEditor>

> 💡 **提示**：`dataclass` 是处理结构化数据的现代 Python 方式，比普通的类更简洁，比字典更安全（支持类型提示）。
