...
`go run` 命令有两种使用方式，对于 JavaScript 开发者来说理解这个区别很重要：
...
go run .
```
- 运行当前目录下的所有 Go 文件（属于同一个包）
- 自动包含同一包内的所有 `.go` 文件
- 适合多文件项目
- 类似于 `npm start` 或运行整个 Node.js 项目

**重要区别：**
- **单文件运行**：只编译和运行指定的文件，忽略同目录下的其他 Go 文件
- **包运行**：编译整个包的所有文件，这是 Go 项目的推荐做法

**示例对比：**
<UniversalEditor title="go run 使用方式对比" compare={true}>
```bash !! bash
# 运行单个文件
go run main.go

# 如果项目有多个文件，可能会出错
# 因为 main.go 可能依赖同包下的其他文件
```
