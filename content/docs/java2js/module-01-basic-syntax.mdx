...
Welcome to your first deep dive into JavaScript syntax! As a Java developer, you'll find many familiar concepts, but JavaScript has some important differences that make it unique.
...
In Java, variables are explicitly typed:
...
JavaScript has 7 primitive types (Java has 8):
...
JavaScript has only one numeric type (unlike Java):
...
JavaScript strings are similar to Java strings but with more flexibility:
...
This is a major difference from Java!
...
JavaScript automatically converts types in certain contexts - this is a major source of bugs!
...
Most operators are similar to Java, but JavaScript has some unique ones:
