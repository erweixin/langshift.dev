          ...
          {t.home.learningPath.modules.map((phase: any, phaseIndex: number) => (
            <div key={phaseIndex} className="relative group">
              ...
              <div className="relative bg-slate-800/30 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8 hover:scale-105 transition-all duration-300 h-full">
                <div className={`w-16 h-16 bg-gradient-to-br ${phaseIndex === 0 ? 'from-amber-500 to-orange-600' : phaseIndex === 1 ? 'from-emerald-500 to-green-600' : 'from-stone-500 to-stone-400'} rounded-2xl flex items-center justify-center mb-6 mx-auto shadow-lg`}>
                  ...
                  {phase.items.map((item: any, itemIndex: number) => (
                    <div key={itemIndex} className="group/item">
                      <div className="flex items-start mb-3">
                        <div className={`w-6 h-6 bg-gradient-to-br ${phaseIndex === 0 ? 'from-amber-500 to-orange-600' : phaseIndex === 1 ? 'from-emerald-500 to-green-600' : 'from-stone-500 to-stone-400'} rounded-full flex items-center justify-center mr-4 mt-1 flex-shrink-0 shadow-sm`}>
                          <CheckCircle className="w-3 h-3 text-white" />
                        </div>
                        <div className="flex-1">
                          <h4 className="text-white font-semibold mb-1 group-hover/item:text-amber-400 transition-colors">
                            {item.title}
                          </h4>
          ...
          <div className="relative">
            ...
            <div className="relative bg-gradient-to-br from-slate-800/30 to-slate-900/30 backdrop-blur-sm border border-slate-700/50 rounded-3xl p-10">
                <div className="text-center mb-8">
                  <div className="w-16 h-16 bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl flex items-center justify-center mb-6 mx-auto shadow-lg">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {t.home.learningPath.learningTips.map((tip: string, index: number) => (
                    <div key={index} className="flex items-start group">
                      <div className="w-8 h-8 bg-gradient-to-br from-amber-500 to-orange-600 rounded-full flex items-center justify-center mr-4 mt-1 flex-shrink-0 shadow-sm group-hover:scale-110 transition-transform">
