...
BEGIN
  ...
  IF to_regprocedure('agent.list_ready_outbox_tenants(uuid,uuid,integer,integer,integer)') IS NULL THEN RAISE EXCEPTION 'ready outbox tenant capability missing'; END IF;
  IF EXISTS (SELECT 1 FROM information_schema.routine_privileges WHERE routine_schema='agent' AND routine_name='list_ready_outbox_tenants' AND grantee='PUBLIC' AND privilege_type='EXECUTE') THEN RAISE EXCEPTION 'ready outbox tenant capability is public'; END IF;
