...
> 本文档定义 Cloud Agent 的首版交付边界。它回答“v1 做什么、不做什么、为什么暂时不做、什么时候再演进”。路线图见 [roadmap.md](./roadmap.md)，部署容量见 [capacity-and-scaling.md](./capacity-and-scaling.md)。

## v1 目标

Lite v1 的目标是交付一个语义正确、可恢复、可观测、可安全运行小规模真实任务的 Cloud Agent 平台。
...
v1 不追求“大规模、多区域、任意复杂编排”。如果最小闭环没有做正确，更高级的基础设施只会把问题放大。
...
简化不等于把语义空着。比如队列可以先用 PostgreSQL，但至少一次投递、去重、超时、重试、死信和反压规则必须定义。
