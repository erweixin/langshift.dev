...
数据库迁移链已扩展到版本 99：账号擦除可以写入内部非识别 locale tombstone `und`，而公开资料写入仍只接受英文与简体中文。聚合 snapshot 对时间窗口和维度参数执行显式 PostgreSQL 类型绑定；完整数据库测试使用全局唯一夹具命名空间，并给确定性故障注入包保留三分钟的 macOS 包级预算。
