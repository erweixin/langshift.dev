...
│   ├── agent_events（append-only，单表）
│   ├── agent_jobs（at-least-once + fence + 原子 append/done，SKIP LOCKED / 单进程队列）
│   ├── 操作事实：agent_idempotency_keys / agent_llm_ledger / content_artifacts（artifact 本体，replay 不清）
│   └── 投影：missions / tasks / profile / evidence …（可由 agent_events 重建）
...
| inbox 去重 | **简化**：`agent_jobs` 表带唯一服务端 command_id；worker 成功路径 append 事件与 job done 同事务 | 单进程下够用，语义不变 |
