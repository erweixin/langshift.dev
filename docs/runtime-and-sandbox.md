...
Workspace Service 不相信 Worker 自报的 fence。EventService / RuntimeManager 在 claim 成功后签发短期 workspace capability，绑定 `tenant_id`、`workspace_id`、`tool_call_id`、`attempt_id`、精确 fence、当前 lease token 的不可逆摘要、base revision、读写范围和过期时间；prepare/publish 时 Workspace Service 都要验证该 capability 和当前 write lease。原始 lease token 只提交给 EventService，不暴露给 sandbox 或 Workspace Service。
...
该协议覆盖所有改变可见 head 的路径，包括普通文件写、人工编辑发布、回滚、cherry-pick 和多 Child Run 的 branch merge。`workspace_merge` 必须先把 branch 集合和 merge policy 规范化为 request hash，生成不可见 prepared merge revision，再走同一 authorization / CommitWorker / reconciliation 流程；任何调用方都没有“直接合并并补记事件”的旁路。
...
5. CommitWorker claim command 后进入 `committing`，拿到绑定 authorization event、revision、attempt/fence/lease-token 摘要和期限的短期 capability，再请求 Workspace Service 用 CAS 发布 revision。Workspace Service 只有验证 durable authorization、当前 head 仍等于 `base_workspace_revision`、workspace lease 和 capability 都匹配时，才能把 revision 变成可见 head。重复发布同一 revision 必须返回同一结果。
