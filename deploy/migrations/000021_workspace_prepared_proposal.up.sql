ALTER TABLE agent.workspace_revision_commits
  ...
  ADD CONSTRAINT workspace_revision_authorization_contract CHECK (
    NULLIF(proposal_hash,'') IS NOT NULL AND (
      (status='prepared' AND approval_id IS NULL AND approval_version IS NULL AND approval_event_id IS NULL
        AND authorization_event_id IS NULL AND commit_command_id IS NULL AND authorized_at IS NULL)
      OR (status<>'prepared' AND status<>'abandoned' AND approval_id IS NOT NULL AND approval_version>0 AND approval_event_id IS NOT NULL
        AND authorization_event_id IS NOT NULL AND commit_command_id IS NOT NULL AND authorized_at IS NOT NULL)
      OR (status='abandoned' AND (
        (approval_id IS NULL AND approval_version IS NULL AND approval_event_id IS NULL AND authorization_event_id IS NULL AND commit_command_id IS NULL AND authorized_at IS NULL)
        OR (approval_id IS NOT NULL AND approval_version>0 AND approval_event_id IS NOT NULL AND authorization_event_id IS NOT NULL AND commit_command_id IS NOT NULL AND authorized_at IS NOT NULL)
      ))
    )
  );
