...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM product.submission_review_generations
    GROUP BY tenant_id,submission_id,rubric_version_id
    HAVING count(*) > 1
  ) THEN
    RAISE EXCEPTION 'cannot roll back review generation retry after retry history exists';
  END IF;
END
$$;

DROP INDEX product.submission_review_generations_active_submission_unique;

ALTER TABLE product.submission_review_generations
  ADD CONSTRAINT submission_review_generations_submission_unique
  UNIQUE (tenant_id,submission_id,rubric_version_id);
