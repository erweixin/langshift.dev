ALTER TABLE agent.runtime_sessions
  ...
  ADD CONSTRAINT runtime_sessions_approval_binding_contract CHECK (
    (approval_id IS NULL AND approval_version IS NULL)
    OR (approval_id IS NOT NULL AND approval_version>0)
  );

CREATE FUNCTION agent.enforce_runtime_session_request_binding() RETURNS trigger
LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.approval_id IS DISTINCT FROM OLD.approval_id
    OR NEW.approval_version IS DISTINCT FROM OLD.approval_version
  THEN
    RAISE EXCEPTION 'immutable runtime session approval binding mutation is forbidden';
  END IF;
  RETURN NEW;
END
$$;

CREATE TRIGGER runtime_session_request_binding_immutable
  BEFORE UPDATE ON agent.runtime_sessions
  FOR EACH ROW EXECUTE FUNCTION agent.enforce_runtime_session_request_binding();
