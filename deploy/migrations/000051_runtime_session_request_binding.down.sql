...
DECLARE approval_required boolean; session_epoch uuid;
BEGIN
  ...
  SELECT p.approval_required,le.store_epoch INTO approval_required,session_epoch
...
DROP TRIGGER IF EXISTS runtime_session_request_binding_immutable ON agent.runtime_sessions;
DROP FUNCTION IF EXISTS agent.enforce_runtime_session_request_binding();
ALTER TABLE agent.runtime_sessions
  DROP CONSTRAINT IF EXISTS runtime_sessions_approval_binding_contract,
