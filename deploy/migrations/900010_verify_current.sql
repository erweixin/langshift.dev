...
BEGIN
  ...
  IF actual<>10 OR NOT EXISTS (SELECT 1 FROM public.lites_schema_migrations WHERE version=10 AND name='effect_reconciliation_contract') THEN
    RAISE EXCEPTION 'expected migration version 10, found % rows',actual;
  END IF;
...
SELECT json_build_object('status','passed','schema_version',10,'table_count',94,'forced_rls_count',86,'effect_reconciliation','single_result_replacement') AS current_schema_verification;
