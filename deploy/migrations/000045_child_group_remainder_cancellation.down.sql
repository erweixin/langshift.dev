DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM agent.child_group_cancellations) THEN
    RAISE EXCEPTION 'child group cancellation facts prevent migration rollback';
  END IF;
END
$$;
...
DROP INDEX IF EXISTS agent.child_group_cancellations_due_idx;
DROP TRIGGER IF EXISTS child_group_cancellation_lifecycle ON agent.child_group_cancellations;
DROP FUNCTION IF EXISTS agent.enforce_child_group_cancellation_lifecycle();
DROP TABLE agent.child_group_cancellations;
