ALTER TABLE agent.run_cancellations
  ADD COLUMN attempt_cancelled_payload_ref text,
  ADD COLUMN attempt_cancelled_payload_hash text,
  ADD COLUMN propagation_cursor uuid,
  ADD COLUMN propagation_complete boolean NOT NULL DEFAULT true,
  ADD COLUMN propagation_command_id uuid,
  ADD COLUMN propagation_payload_ref text,
  ADD COLUMN propagation_payload_hash text;

UPDATE agent.run_cancellations
SET attempt_cancelled_payload_ref=settlement_payload_ref,
    attempt_cancelled_payload_hash=settlement_payload_hash;

ALTER TABLE agent.run_cancellations
  ADD CONSTRAINT run_cancellations_attempt_payload_contract CHECK (
    (attempt_cancelled_payload_ref IS NULL AND attempt_cancelled_payload_hash IS NULL)
    OR (NULLIF(attempt_cancelled_payload_ref,'') IS NOT NULL
      AND attempt_cancelled_payload_hash ~ '^[0-9a-f]{64}$')
  ),
  ADD CONSTRAINT run_cancellations_propagation_contract CHECK (
    (propagation_command_id IS NULL AND propagation_payload_ref IS NULL AND propagation_payload_hash IS NULL)
    OR (propagation_command_id IS NOT NULL AND NULLIF(propagation_payload_ref,'') IS NOT NULL
      AND propagation_payload_hash ~ '^[0-9a-f]{64}$')
  ),
  ADD CONSTRAINT run_cancellations_propagation_command_fk FOREIGN KEY (tenant_id,propagation_command_id)
...
CREATE INDEX run_cancellations_propagation_idx
  ON agent.run_cancellations(tenant_id,id)
  WHERE status='terminating' AND NOT propagation_complete;
