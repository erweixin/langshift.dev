...
BEGIN
  ...
  END IF;
  IF OLD.result_event_id IS NOT NULL AND NEW.result_event_id IS DISTINCT FROM OLD.result_event_id
    AND NOT (OLD.status='outcome_unknown' AND NEW.status IN ('confirmed','failed','commit_authorized','accepted_unknown')) THEN
  ...
  IF NOT (
    ...
    OR (OLD.status='outcome_unknown' AND NEW.status='outcome_unknown'
      AND NEW.reconciliation_due_at>OLD.reconciliation_due_at
      AND NEW.result_event_id IS NOT DISTINCT FROM OLD.result_event_id)
  ) THEN
