...
BEGIN
  ...
  IF NEW.id IS DISTINCT FROM OLD.id OR NEW.tenant_id IS DISTINCT FROM OLD.tenant_id
    ...
    OR ((NEW.settlement_payload_ref IS DISTINCT FROM OLD.settlement_payload_ref
      ...
      AND NOT (OLD.status='terminating' AND NEW.status='settled' AND OLD.parent_cancellation_id IS NULL))
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE e.id=NEW.request_event_id AND e.tenant_id=NEW.tenant_id
      ...
      AND e.payload_ref=NEW.request_payload_ref AND e.payload_hash=NEW.request_payload_hash
  ...
  IF NEW.status='settled' AND NOT EXISTS (
    ...
    WHERE e.id=NEW.settlement_event_id AND e.tenant_id=NEW.tenant_id
      ...
      AND e.aggregate_id=NEW.run_id AND e.aggregate_version=r.run_version
      AND e.payload_ref=NEW.settlement_payload_ref AND e.payload_hash=NEW.settlement_payload_hash
  ...
  ) THEN RAISE EXCEPTION 'settled run cancellation lacks matching terminal event and payload'; END IF;
  RETURN NULL;
END
$$;
