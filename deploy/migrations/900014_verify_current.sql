...
BEGIN
  ...
  IF actual<>14 OR NOT EXISTS (SELECT 1 FROM public.lites_schema_migrations WHERE version=14 AND name='repair_resolution_contract') THEN
    RAISE EXCEPTION 'expected migration version 14, found % rows',actual;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='agent.repair_approvals'::regclass AND conname='repair_approvals_event_fk' AND condeferrable AND condeferred) THEN RAISE EXCEPTION 'repair approval event is not deferred'; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='agent.repair_commands'::regclass AND conname='repair_commands_lifecycle_contract') THEN RAISE EXCEPTION 'repair lifecycle shape missing'; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='agent.repair_approvals'::regclass AND conname='repair_approvals_scope_contract') THEN RAISE EXCEPTION 'repair approval scope missing'; END IF;
  ...
  IF position('terminal repair command mutation is forbidden' IN lifecycle)=0 OR position('NEW.version<>OLD.version+1' IN lifecycle)=0 THEN
    RAISE EXCEPTION 'repair command lifecycle fence missing';
  END IF;
  IF to_regclass('agent.repair_commands_pending_idx') IS NULL THEN RAISE EXCEPTION 'repair pending index missing'; END IF;
...
SELECT json_build_object('status','passed','schema_version',14,'table_count',94,'forced_rls_count',86,'repair_resolution','two_person_fenced') AS current_schema_verification;
