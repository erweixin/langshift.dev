...
for cycle in $(seq 1 "${cycles}"); do
  ...
  if ALLOW_INSECURE_DEVELOPMENT=true DATABASE_URL="${database_url}" "${work}/lites-migrate" -direction down -steps 2 >/dev/null 2>&1; then
  ...
  [[ "${status_output}" == *'"current_version":2'* ]] || { printf '%s\n' "${status_output}"; exit 1; }
