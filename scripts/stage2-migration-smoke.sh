...
for cycle in $(seq 1 "${cycles}"); do
  ...
  docker cp deploy/migrations/900036_verify_current.sql "${current_container}:/tmp/verify.sql" >/dev/null
  ...
  if ALLOW_INSECURE_DEVELOPMENT=true DATABASE_URL="${database_url}" "${work}/lites-migrate" -direction down -steps 36 >/dev/null 2>&1; then
  ...
  [[ "${status_output}" == *'"current_version":36'* ]] || { printf '%s\n' "${status_output}"; exit 1; }
