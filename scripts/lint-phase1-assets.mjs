...
check("DATABASE-EXECUTABLE-VERIFICATION",["expected 90 contract tables","expected 83 forced-RLS tables","expected 21 append-only triggers","cross-tenant row became visible","append-only mutation unexpectedly succeeded","claim_key uniqueness missing","session active tenant binding missing","session CAS version missing","session active tenant foreign key missing","idempotency response scope uniqueness missing","event aggregate version uniqueness missing","Mission Focus primary key missing","effect ledger uniqueness missing","two-person distinct-vote constraint missing","credit conservation check missing"].every(marker=>databaseVerification.includes(marker)),"database verification covers catalog, isolation, immutability and critical domain constraints");
...
const eventTable=database.tables.find(table=>table.qualifiedName==="agent.events");
check("DATABASE-EVENT-AGGREGATE-CAS",eventTable?.appendOnly&&eventTable.uniques.some(columns=>JSON.stringify(columns)===JSON.stringify(["tenant_id","aggregate_kind","aggregate_id","aggregate_version"])),"event store rejects concurrent duplicate aggregate versions at the database boundary");
