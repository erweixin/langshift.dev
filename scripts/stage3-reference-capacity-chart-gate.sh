...
runtime="$(mktemp -d "${TMPDIR:-/tmp}/lites-capacity-chart.XXXXXX")"
...
chart="deploy/helm/reference-capacity"
...
docker run --rm --volume "${root}:/workspace:ro" --volume "${runtime}:/output" --workdir /workspace "${helm_image}" template reference-capacity "${chart}" --namespace load-system --values "${fixture}" --output-dir /output >/dev/null
rendered="${runtime}/lites-reference-capacity/templates/resources.yaml"
...
node "${root}/scripts/validate-stage3-reference-capacity-render.mjs" "${rendered}"
...
printf 'stage-3 reference capacity chart security: high_critical_misconfigurations=0 status=passed\n'
