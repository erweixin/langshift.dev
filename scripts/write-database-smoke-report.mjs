...
const report={
  ...
  results:{freshInstall:"passed",tableCount:Number(process.env.TABLE_COUNT),forcedRlsCount:Number(process.env.FORCED_RLS_COUNT),appendOnlyTriggerCount:Number(process.env.APPEND_ONLY_TRIGGER_COUNT),crossTenantVisibleRows:0,appendOnlyMutationsSucceeded:0,criticalConstraintsVerified:8},
  criticalConstraints:["anonymous claim_key uniqueness","Mission Focus tenant-user primary key","Route claim_set_hash","Route base_route_version","reminder delivery dedupe","effect ledger uniqueness","distinct repair approver vote","credit conservation check"],
