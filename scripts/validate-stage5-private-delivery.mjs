...
const currentMigration = migrationManifest.migrations.at(-1);
check("MIGRATION-CLOSURE", Number.isInteger(currentMigration?.version) && currentMigration.version > 0 && typeof currentMigration.name === "string" && currentMigration.name.length > 0 && install.includes("900xxx_verify_current.sql") && install.includes("migration lock"), `delivery binds current migration ${currentMigration?.version ?? "missing"} and the post-migration verifier`);
