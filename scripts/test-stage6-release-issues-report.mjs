...
import { githubIssueRequestContract, normalizeGitHubIssuePages } from "./collect-stage6-release-issues.mjs";
...
const issue = (number, state, labels) => ({ node_id: `I_${number}`, number, title: `Issue ${number}`, html_url: `https://github.com/erweixin/langshift.dev/issues/${number}`, state, created_at: capturedAt, updated_at: capturedAt, closed_at: state === "closed" ? capturedAt : null, labels: labels.map((name) => ({ name })) });
const pages = [{ page: 1, items: [issue(1, "open", ["severity:P2"]), issue(2, "closed", ["severity:P0"])], hasNextPage: false }];
const snapshot = normalizeGitHubIssuePages(pages, { repository: "erweixin/langshift.dev", sourceCommit, rcHash, capturedAt });
assert.equal(snapshot.querySha256, sha256(githubIssueRequestContract));
const policy = { policyVersion: "1.0.0", repository: "erweixin/langshift.dev", provider: "github-rest", maximumSnapshotAgeHours: 24, severityLabels: { P0: "severity:P0", P1: "severity:P1", P2: "severity:P2", P3: "severity:P3" }, pullRequestsExcluded: true };
...
const incomplete = structuredClone(pages); incomplete[0].hasNextPage = true;
