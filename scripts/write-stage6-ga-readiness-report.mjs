...
const issuesPath = "release-candidates/current/release-issues.json";
const issueSnapshot = await blobEvidence("release-candidates/current/evidence/release-issues-snapshot.json");
const issues = await signedJsonEvidence(issuesPath, "automatedReports", (value) => value.reportVersion === "1.0.0" && value.kind === "release-issues" && value.status === "passed" && value.worktreeDirty === false && value.sourceCommit === rc.value?.sourceCommit && value.openP0 === 0 && value.openP1 === 0 && value.unclassifiedOpen === 0 && value.snapshot?.paginationComplete === true && value.snapshot?.sha256 === issueSnapshot.hash && Number.isFinite(Date.parse(value.generatedAt)) && Date.now() >= Date.parse(value.generatedAt) && Date.now() - Date.parse(value.generatedAt) <= 30 * 24 * 60 * 60 * 1000 && value.rcHash === rc.value?.releaseCandidateHash);
check("RELEASE-ISSUES", issues.passed && issues.signatureVerified ? "passed" : "pending", "the signed RC issue inventory must contain zero open P0/P1 issues", signedDetails(issues, issuesPath));
