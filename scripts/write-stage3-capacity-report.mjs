...
const requiredEvidenceKeys = ["deploymentManifestSha256", "imageDigestsSha256", "hardwareTopologySha256", "providerEmulatorSha256", "runtimeImageDigest", "datasetManifestSha256", "prometheusSnapshotSha256"];

export function validateCapacityResult(profile, result, sourceCommit) {
  const failures = [];
  ...
  if (result.environment?.topology !== "reference_production" || result.environment?.highAvailability !== true || result.environment?.localFoundation === true) failures.push("production_topology");
  ...
  for (const [name, minimum] of Object.entries(profile.vectors)) {
    ...
    if (!measurement || !Number.isFinite(measurement.achieved) || measurement.achieved < minimum || measurement.target !== minimum || measurement.observations < 1 || !measurement.prometheusQuery || !/^[0-9a-f]{64}$/.test(measurement.rawSeriesSha256 ?? "")) failures.push(`vector:${name}`);
  }
  for (const [name, threshold] of Object.entries(profile.slos)) {
    const measurement = result.slos?.[name];
    let passes = measurement && Number.isFinite(measurement.value) && measurement.observations > 0 && measurement.prometheusQuery && /^[0-9a-f]{64}$/.test(measurement.rawSeriesSha256 ?? "");
  ...
  for (const name of profile.zeroTolerance) {
    ...
    if (!measurement || measurement.value !== 0 || measurement.observations < 1 || !measurement.prometheusQuery || !/^[0-9a-f]{64}$/.test(measurement.rawSeriesSha256 ?? "")) failures.push(`zero_tolerance:${name}`);
  }
...
async function main() {
  ...
  const profileRaw = await readFile("load-tests/stage3/reference-capacity-profile.json");
  const raw = await readFile(input);
  const profile = JSON.parse(profileRaw);
  const result = JSON.parse(raw);
  const failures = validateCapacityResult(profile, result, sourceCommit);
