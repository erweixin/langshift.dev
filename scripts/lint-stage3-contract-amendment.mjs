...
check("AMENDMENT-V1.6-EVENTS",JSON.stringify(v16EventNames)===JSON.stringify(["MemoryUpsertedV2","MemoryDeletedV2","MemoryIndexProjectionIndexed","RetrievalManifestCommitted"])&&v16EventNames.every(name=>!baseNames.has(name)&&!eventNames.includes(name)&&!v12EventNames.includes(name)&&!v13EventNames.includes(name)&&!v14EventNames.includes(name)&&!v15EventNames.includes(name)),"Memory lifecycle, index projection and retrieval manifest facts add without replacing frozen registry keys");
...
const memoryIndexed=v16Registry.schemas.MemoryIndexProjectionIndexed.properties.payload.required;
const retrievalCommitted=v16Registry.schemas.RetrievalManifestCommitted.properties.payload.required;
check("MEMORY-UPSERT-REBUILD",["memory_scope","scope_id","memory_kind","content_ref","content_hmac","content_type","source_kind","trust_label","source_refs","data_subject_ids","derived_from_memory_revisions","derivation_kind","encryption_subject_id","key_ref","embedding_model_id","tool_call_id","tool_call_version","policy_version","guardrail_snapshot_id","index_generation","legacy_incomplete"].every(field=>memoryUpserted.includes(field)),"Memory upsert v2 carries encrypted content, governed ToolCall, immutable trust and complete erasure/rebuild lineage");
...
check("MEMORY-INDEX-EVIDENCE",["memory_id","memory_version","index_generation","embedding_model_id","embedding_model_version","vector_dimensions","embedding_ref","lexical_ref","indexed_at"].every(field=>memoryIndexed.includes(field)),"Memory index completion binds exact revision, generation, model and external projection references");
