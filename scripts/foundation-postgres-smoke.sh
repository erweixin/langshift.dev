...
if [[ -n "${LITES_FOUNDATION_TEST_JSON:-}" ]]; then
  go test -json -count=1 -timeout=60s -tags=integration ./internal/identity/postgres ./internal/identity/mail ./internal/eventstore/postgres ./internal/execution/postgres ./internal/product/postgres | tee "${LITES_FOUNDATION_TEST_JSON}"
else
  go test -count=1 -timeout=60s -tags=integration ./internal/identity/postgres ./internal/identity/mail ./internal/eventstore/postgres ./internal/execution/postgres ./internal/product/postgres
fi
