...
docker cp deploy/migrations/900087_verify_current.sql "${container_name}:/tmp/900087_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900087_verify_current.sql >/dev/null
...
test_timeout="${LITES_FOUNDATION_TEST_TIMEOUT:-60s}"
