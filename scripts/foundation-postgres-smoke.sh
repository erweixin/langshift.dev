...
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -c \
  "GRANT SELECT,UPDATE ON identity.onboarding_sessions TO lites_product_service; GRANT SELECT,INSERT ON identity.onboarding_claims TO lites_product_service; GRANT EXECUTE ON FUNCTION identity.list_onboarding_route_reconciliation_tenants(uuid,integer) TO lites_product_service;" >/dev/null
...
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -c \
  "GRANT SELECT ON identity.tenants TO lites_product_service; GRANT SELECT,INSERT,UPDATE ON product.programs,product.cohorts,product.enrollments TO lites_product_service; GRANT SELECT,INSERT ON product.role_packs,product.task_packs TO lites_product_service; GRANT SELECT ON product.task_templates TO lites_product_service;" >/dev/null

docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -c \
  "GRANT SELECT,INSERT,UPDATE ON product.support_cases TO lites_product_service; GRANT SELECT,INSERT ON product.support_case_messages TO lites_product_service; GRANT EXECUTE ON FUNCTION product.resolve_support_sla(uuid,text) TO lites_product_service;" >/dev/null

docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -c \
  "GRANT SELECT ON identity.sessions TO lites_product_service; GRANT SELECT,INSERT,UPDATE ON product.byok_credentials,product.memory_policies TO lites_product_service; GRANT SELECT,INSERT ON product.byok_credential_versions,product.settings_access_audit TO lites_product_service;" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -c \
  "GRANT SELECT ON identity.users,identity.memberships TO lites_product_service; GRANT SELECT ON product.daily_tasks,product.projects,product.enrollments,product.cohorts,product.role_packs TO lites_product_service; GRANT SELECT,INSERT ON product.aggregate_metrics,product.aggregate_snapshots TO lites_product_service; GRANT USAGE ON SCHEMA contracts TO lites_product_service; GRANT SELECT ON contracts.usage_ledger TO lites_product_service;" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -c \
  "CREATE ROLE lites_contract_service LOGIN PASSWORD 'foundation_contract_service' NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOBYPASSRLS; GRANT CONNECT ON DATABASE lites_foundation TO lites_contract_service; GRANT USAGE ON SCHEMA identity,agent,contracts TO lites_contract_service; GRANT SELECT ON identity.tenants,identity.memberships,identity.sessions TO lites_contract_service; GRANT SELECT ON contracts.contracts,contracts.seat_allocations,contracts.contract_audit_events,contracts.credit_buckets,contracts.manual_adjustments,contracts.contract_entitlements TO lites_contract_service; GRANT SELECT,INSERT ON contracts.admin_access_audit,contracts.audit_exports TO lites_contract_service; GRANT SELECT,INSERT ON contracts.contract_proposals,contracts.contract_approval_decisions,contracts.accounting_adjustment_proposals,contracts.accounting_adjustment_approval_decisions,contracts.entitlement_proposals,contracts.entitlement_approval_decisions TO lites_contract_service; GRANT SELECT,INSERT,UPDATE ON agent.idempotency_responses,agent.event_cursors,agent.outbox TO lites_contract_service; GRANT SELECT,INSERT ON agent.events TO lites_contract_service; GRANT EXECUTE ON FUNCTION contracts.apply_approved_contract_proposal(uuid,uuid,uuid,uuid,timestamptz),contracts.apply_approved_accounting_adjustment(uuid,uuid,uuid,uuid,timestamptz),contracts.apply_approved_entitlement_proposal(uuid,uuid,uuid,uuid,uuid,timestamptz) TO lites_contract_service;" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -c \
  "GRANT SELECT ON agent.llm_provider_attempts TO lites_contract_service; GRANT SELECT,INSERT,UPDATE ON contracts.usage_reservations TO lites_contract_service; GRANT SELECT,INSERT ON contracts.usage_ledger TO lites_contract_service; GRANT EXECUTE ON FUNCTION contracts.reserve_credit_units(uuid,uuid,bigint,timestamptz,timestamptz),contracts.settle_credit_units(uuid,uuid,bigint,bigint,timestamptz),contracts.release_credit_units(uuid,uuid,bigint,timestamptz) TO lites_contract_service;" >/dev/null
...
docker cp deploy/migrations/900088_verify_current.sql "${container_name}:/tmp/900088_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900088_verify_current.sql >/dev/null
docker cp deploy/migrations/900089_verify_current.sql "${container_name}:/tmp/900089_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900089_verify_current.sql >/dev/null
docker cp deploy/migrations/900090_verify_current.sql "${container_name}:/tmp/900090_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900090_verify_current.sql >/dev/null
docker cp deploy/migrations/900091_verify_current.sql "${container_name}:/tmp/900091_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900091_verify_current.sql >/dev/null
docker cp deploy/migrations/900092_verify_current.sql "${container_name}:/tmp/900092_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900092_verify_current.sql >/dev/null
docker cp deploy/migrations/900093_verify_current.sql "${container_name}:/tmp/900093_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900093_verify_current.sql >/dev/null
docker cp deploy/migrations/900096_verify_current.sql "${container_name}:/tmp/900096_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900096_verify_current.sql >/dev/null
docker cp deploy/migrations/900097_verify_current.sql "${container_name}:/tmp/900097_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900097_verify_current.sql >/dev/null
docker cp deploy/migrations/900098_verify_current.sql "${container_name}:/tmp/900098_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900098_verify_current.sql >/dev/null
docker cp deploy/migrations/900099_verify_current.sql "${container_name}:/tmp/900099_verify_current.sql" >/dev/null
docker exec "${container_name}" psql -v ON_ERROR_STOP=1 -U postgres -d lites_foundation -f /tmp/900099_verify_current.sql >/dev/null
...
if [[ -n "${LITES_FOUNDATION_TEST_JSON:-}" ]]; then
  if [[ "${LITES_FOUNDATION_TEST_QUIET:-false}" == "true" ]]; then
    go test -p=1 -json -count=1 -timeout="${test_timeout}" -tags=integration "${test_run_args[@]}" "${test_packages[@]}" >"${LITES_FOUNDATION_TEST_JSON}"
  else
    go test -p=1 -json -count=1 -timeout="${test_timeout}" -tags=integration "${test_run_args[@]}" "${test_packages[@]}" | tee "${LITES_FOUNDATION_TEST_JSON}"
  fi
else
  go test -p=1 -count=1 -timeout="${test_timeout}" -tags=integration "${test_run_args[@]}" "${test_packages[@]}"
fi
