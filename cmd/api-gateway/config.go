...
type config struct {
	...
	contractURL, contractCAFile, contractCertFile, contractKeyFile, contractTLSServerName          string
	secretBundleFile, trustedIssuer, trustedAudience, realtimeTrustedAudience                      string
	behaviorTrustedAudience, agentTrustedAudience, productTrustedAudience, contractTrustedAudience string
...
func loadConfig() (config, error) {
	...
	value := config{
		...
		contractURL: os.Getenv("CONTRACT_UPSTREAM_URL"), contractCAFile: os.Getenv("CONTRACT_UPSTREAM_ROOT_CA_FILE"), contractCertFile: os.Getenv("CONTRACT_UPSTREAM_CLIENT_CERT_FILE"), contractKeyFile: os.Getenv("CONTRACT_UPSTREAM_CLIENT_KEY_FILE"), contractTLSServerName: os.Getenv("CONTRACT_UPSTREAM_TLS_SERVER_NAME"),
		secretBundleFile: os.Getenv("GATEWAY_SECRET_BUNDLE_FILE"), trustedKeyringFile: os.Getenv("TRUSTED_CONTEXT_KEYRING_FILE"), trustedIssuer: envString("TRUSTED_CONTEXT_ISSUER", "lites-gateway"), trustedAudience: envString("TRUSTED_CONTEXT_AUDIENCE", "identity-service"), realtimeTrustedAudience: envString("REALTIME_TRUSTED_CONTEXT_AUDIENCE", "realtime-gateway"), behaviorTrustedAudience: envString("BEHAVIOR_TRUSTED_CONTEXT_AUDIENCE", "behavior-control-plane"), agentTrustedAudience: envString("AGENT_TRUSTED_CONTEXT_AUDIENCE", "agent-control-plane"), productTrustedAudience: envString("PRODUCT_TRUSTED_CONTEXT_AUDIENCE", "product-service"), contractTrustedAudience: envString("CONTRACT_TRUSTED_CONTEXT_AUDIENCE", "contract-service"), trustedContextTTL: contextTTL, publicOrigins: splitNonempty(os.Getenv("PUBLIC_ORIGINS")), trustedProxyCIDRs: splitNonempty(os.Getenv("TRUSTED_PROXY_CIDRS")), allowInsecureDevelopment: allowInsecure,
...
func (value config) validate() error {
	...
	if !value.allowInsecureDevelopment && (value.databaseURLFile == "" || value.serverCertificateFile == "" || value.identityCAFile == "" || value.identityCertFile == "" || value.identityTLSServerName == "" || value.realtimeCAFile == "" || value.realtimeCertFile == "" || value.realtimeTLSServerName == "" || value.behaviorCAFile == "" || value.behaviorCertFile == "" || value.behaviorTLSServerName == "" || value.agentCAFile == "" || value.agentCertFile == "" || value.agentTLSServerName == "" || value.productCAFile == "" || value.productCertFile == "" || value.productTLSServerName == "" || value.contractCAFile == "" || value.contractCertFile == "" || value.contractTLSServerName == "" || value.otlpEndpoint == "" || (value.otlpBearerTokenFile == "" && value.otlpCertFile == "")) {
...
func (value config) contractUpstreamClient() (*http.Client, *url.URL, error) {
	return configuredUpstreamClient(value.contractURL, value.contractCAFile, value.contractCertFile, value.contractKeyFile, value.contractTLSServerName, 35*time.Second)
}
