...
var behaviorUUIDPattern = regexp.MustCompile(`^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$`)

type config struct {
	databaseURL, databaseURLFile                                                          string
	adminAddress, rollbackAddress, healthAddress                                          string
	serverCertificateFile, serverKeyFile, serverClientCAFile                              string
	rollbackAllowedSPIFFEID                                                               string
	trustedKeyringFile, trustedIssuer, trustedAudience, behaviorKeyringFile               string
	secretBundleFile, automationUserID                                                    string
	...
	idempotencyTTL, reauthenticationAge                                                   time.Duration
...
func loadConfig() (config, error) {
	...
	reauthenticationAge, err := optionalDuration("PROMOTION_REAUTHENTICATION_AGE", 15*time.Minute)
	if err != nil {
		return config{}, err
	}
	...
	value := config{
		...
		adminAddress: env("ADMIN_LISTEN_ADDRESS", ":8443"), rollbackAddress: env("ROLLBACK_LISTEN_ADDRESS", ":8444"), healthAddress: env("HEALTH_ADDRESS", "127.0.0.1:8087"),
		serverCertificateFile: os.Getenv("SERVER_TLS_CERT_FILE"), serverKeyFile: os.Getenv("SERVER_TLS_KEY_FILE"), serverClientCAFile: os.Getenv("SERVER_CLIENT_CA_FILE"), rollbackAllowedSPIFFEID: os.Getenv("ROLLBACK_ALLOWED_CLIENT_SPIFFE_ID"),
		trustedKeyringFile: os.Getenv("TRUSTED_CONTEXT_KEYRING_FILE"), trustedIssuer: env("TRUSTED_CONTEXT_ISSUER", "lites-gateway"), trustedAudience: env("TRUSTED_CONTEXT_AUDIENCE", "behavior-control-plane"), behaviorKeyringFile: os.Getenv("BEHAVIOR_SIGNING_KEYRING_FILE"),
		secretBundleFile: os.Getenv("BEHAVIOR_SECRET_BUNDLE_FILE"), automationUserID: os.Getenv("BEHAVIOR_AUTOMATION_USER_ID"),
		...
		idempotencyTTL: idempotencyTTL, reauthenticationAge: reauthenticationAge, traceSampleRatio: traceSampleRatio, allowInsecureDevelopment: allow,
	}
	return value, value.validate()
}

func (value config) validate() error {
	required := []string{value.databaseURL, value.adminAddress, value.rollbackAddress, value.healthAddress, value.trustedKeyringFile, value.trustedIssuer, value.trustedAudience, value.behaviorKeyringFile, value.secretBundleFile, value.epochURL, value.s3Region, value.payloadBucket, value.vaultAddress, value.vaultMount, value.vaultKeyPrefix, value.environment, value.serviceVersion, value.region}
	for _, item := range required {
		if item == "" {
			return errors.New("required behavior control-plane configuration is missing")
		}
	}
	if !behaviorUUIDPattern.MatchString(value.automationUserID) || value.adminAddress == value.rollbackAddress || value.adminAddress == value.healthAddress || value.rollbackAddress == value.healthAddress || value.databaseMaxConnections < 8 || value.databaseMaxConnections > 256 || value.idempotencyTTL < time.Hour || value.idempotencyTTL > 7*24*time.Hour || value.reauthenticationAge <= 0 || value.reauthenticationAge > time.Hour || value.traceSampleRatio < 0 || value.traceSampleRatio > 1 || (value.serverCertificateFile == "") != (value.serverKeyFile == "") || (value.epochCertFile == "") != (value.epochKeyFile == "") || (value.vaultCertFile == "") != (value.vaultKeyFile == "") || (value.otlpCertFile == "") != (value.otlpKeyFile == "") {
		return errors.New("behavior control-plane configuration is invalid")
	}
	identity, err := url.Parse(value.rollbackAllowedSPIFFEID)
	if err != nil || identity.Scheme != "spiffe" || identity.Host == "" || identity.User != nil || identity.RawQuery != "" || identity.Fragment != "" {
		return errors.New("rollback SPIFFE identity is invalid")
	}
	epochEndpoint, err := url.Parse(value.epochURL)
	if err != nil || epochEndpoint.Host == "" || epochEndpoint.User != nil || (!value.allowInsecureDevelopment && epochEndpoint.Scheme != "https") || value.allowInsecureDevelopment && epochEndpoint.Scheme == "http" && !loopback(epochEndpoint.Hostname()) {
	...
	if value.s3Encryption == types.ServerSideEncryptionAwsKms && value.s3KMSKeyID == "" || value.s3Encryption != types.ServerSideEncryptionAwsKms && value.s3Encryption != types.ServerSideEncryptionAes256 {
		return errors.New("behavior payload S3 encryption is invalid")
	}
	if !value.allowInsecureDevelopment && (value.databaseURLFile == "" || value.serverCertificateFile == "" || value.serverClientCAFile == "" || value.epochCAFile == "" || value.epochTokenFile == "" && value.epochCertFile == "" || value.vaultCAFile == "" || value.vaultTokenFile == "" && value.vaultCertFile == "" || value.otlpEndpoint == "" || value.otlpBearerTokenFile == "" && value.otlpCertFile == "") {
		return errors.New("production behavior control plane requires file-backed authenticated TLS dependencies")
	}
	return nil
}
...
func loadSecretBundle(path string) (secretBundle, error) {
	...
	if err != nil {
		return secretBundle{}, errors.New("behavior secret bundle is unavailable")
	}
	defer file.Close()
	encoded, err := io.ReadAll(io.LimitReader(file, 16*1024+1))
	if err != nil || len(encoded) > 16*1024 {
		return secretBundle{}, errors.New("behavior secret bundle is invalid")
	}
	...
	for index, value := range values {
		...
		if err != nil || len(decoded[index]) != 32 {
			return secretBundle{}, errors.New("behavior secret bundle material is invalid")
		}
		fingerprint := base64.StdEncoding.EncodeToString(decoded[index])
		if _, duplicate := seen[fingerprint]; duplicate {
			return secretBundle{}, errors.New("behavior secret bundle reuses purpose-separated material")
		}
