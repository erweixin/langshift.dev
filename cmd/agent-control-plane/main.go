...
func main() {
	...
	if err = run(ctx, configuration, logger); err != nil {
		logger.Error("agent control plane stopped", "error", err)
		os.Exit(1)
	}
}
...
func run(parent context.Context, configuration config, logger *slog.Logger) error {
	...
	store := executionpostgres.RunStore{
		...
		StoreEpoch: storeEpoch, Epochs: authority, Tokens: opaque.Manager{Purpose: "agent-control-run-lease", Pepper: secrets.LeaseTokenPepper},
		LeaseTTL: configuration.leaseTTL, Behavior: behaviorpostgres.Store{},
	}
	controlService := executionpostgres.ControlService{
	...
	controlHandler := executionapi.ControlHandler{Service: controlService}
	approvalHandler := executionapi.ApprovalHandler{Service: approvalService}
	repairHandler := executionapi.RepairHandler{Service: repairService}
	router := http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		switch {
		case strings.HasPrefix(request.URL.Path, "/v1/approvals/"), strings.HasPrefix(request.URL.Path, "/v1/admin/approval-requests/"):
			approvalHandler.ServeHTTP(writer, request)
		case request.URL.Path == "/v1/admin/repair-commands", strings.HasPrefix(request.URL.Path, "/v1/admin/repair-commands/"):
			repairHandler.ServeHTTP(writer, request)
		default:
			controlHandler.ServeHTTP(writer, request)
		}
	})
	application := serviceauth.Middleware{Verifier: trustedcontext.Verifier{Issuer: configuration.trustedIssuer, Audience: configuration.trustedAudience, Keys: trustedKeys, KeyWindows: trustedWindows, MaximumTTL: 2 * time.Minute, ClockSkew: 5 * time.Second}, Now: now}.Wrap(router)
	serverTLS, err := serverTLSConfig(configuration)
	if err != nil {
		return err
	}
	server := &http.Server{Addr: configuration.listenAddress, Handler: telemetry.WrapHTTP(application), TLSConfig: serverTLS, ReadHeaderTimeout: 3 * time.Second, ReadTimeout: 20 * time.Second, WriteTimeout: 45 * time.Second, IdleTimeout: time.Minute, MaxHeaderBytes: 32 << 10}
	ready := &atomic.Bool{}
	ready.Store(true)
	health := controlHealth(configuration.healthAddress, ready, pool, authority, storeEpoch, blobs, vaultReader, telemetry.MetricsHandler())
	errChannel := make(chan error, 3)
	go serveHTTP(server, serverTLS, errChannel)
	go func() {
	...
	logger.Info("agent control plane ready", "address", configuration.listenAddress, "store_epoch", storeEpoch, "region", configuration.region)
	var runErr error
	select {
	case <-parent.Done():
...
func dependencyHTTPClient(caFile, certFile, keyFile string) (*http.Client, error) {
	...
	if caFile != "" {
		...
		roots := x509.NewCertPool()
		if !roots.AppendCertsFromPEM(contents) {
			return nil, errors.New("dependency CA is invalid")
		}
...
func controlHealth(address string, ready *atomic.Bool, pool *pgxpool.Pool, authority eventpostgres.EpochAuthority, expectedEpoch string, blobs s3store.Store, vault *vaultkeys.ClientReader, metrics http.Handler) *http.Server {
