...
func run(parent context.Context, configuration config, logger *slog.Logger) error {
	...
	runStore := executionpostgres.RunStore{Pool: agentPool, Appender: appender, IDKey: executionIDKey, StoreEpoch: storeEpoch, Epochs: authority, Tokens: leaseTokens, LeaseTTL: configuration.executionLeaseTTL, RequireDispatchFence: true}
	sessionStore := sessionrequest.Store{Pool: agentPool, Appender: appender, Epochs: authority, StoreEpoch: storeEpoch, IDKey: runtimeIDKey, NonceKey: runtimeNonceKey, ExecutionTokens: leaseTokens}
